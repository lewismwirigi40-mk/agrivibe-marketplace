...
export default function GuideDetail() {
  ...
  // ✅ Fetch guide and check purchase status on load
  useEffect(() => {
    if (slug) {
      fetchGuide();
      checkPurchaseStatus();
  ...
  const checkPurchaseStatus = async () => {
    try {
      ...
      if (guide?.id) {
        const response = await api.get(`/guides/purchase-status/${guide.id}`);
        if (response.data.purchased) {
          setPurchased(true);
          setDownloadToken(response.data.purchase?.download_token || "");
        } else {
          setPurchased(false);
        }
      }
      ...
      try {
        ...
        const found = purchases.find((p: any) => p.guide?.slug === slug);
        if (found) {
          setPurchased(true);
          setDownloadToken(found.download_token);
        }
      } catch (e) {
        // Ignore
      }
  ...
  // ✅ FORCE re-check purchase status (called after payment)
  ...
  const handlePaymentComplete = async () => {
    try {
      ...
      const response = await api.post("/guides/confirm", {
        ...
        mpesa_code: "MPESA-" + Date.now(),
      });

      if (response.data.success || response.data.message) {
        setShowSuccess(true);
        setSuccessMessage(
          "✅ Payment confirmed! Check your email for download link.",
        );
        setPurchased(true);
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      // ✅ If wallet payment, auto-complete
      if (paymentMethod === "wallet" && response.data.status === "completed") {
        setShowSuccess(true);
        setSuccessMessage("✅ Purchase completed! Check your email.");
        setPurchased(true);
        setDownloadToken(response.data.download_token);
        await refreshPurchaseStatus();
        setTimeout(() => {
      ...
      if (error.response?.data?.error?.includes("already purchased")) {
        setPurchased(true);
        setError("You have already purchased this guide.");
        await refreshPurchaseStatus();
      } else {
  ...
  const handleDownload = () => {
    if (downloadToken) {
      const downloadUrl = `${api.defaults.baseURL}/guides/download/${downloadToken}`;
      ...
      <div className="min-h-screen bg-premium-light">
        ...
        <div className="container-premium pt-32 pb-16">
          <div className="text-center py-20">
            ...
            <Link
              href="/guides"
              className="inline-flex items-center gap-2 mt-4 text-agrivibe-green hover:text-emerald-600 transition-colors"
            >
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                ...
                {showSuccess && (
                  ...
                  >
                    ...
                    {downloadToken && (
                      <button
                        onClick={handleDownload}
                        className="mt-3 w-full bg-gradient-to-r from-agrivibe-green to-emerald-500 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-xl transition-all"
                      >
                ...
                {purchased && !showSuccess ? (
                  <div className="mt-4 p-4 bg-green-50 rounded-xl border border-green-200">
                    ...
                    <button
                      onClick={handleDownload}
                      className="mt-3 w-full flex items-center justify-center gap-2 bg-gradient-to-r from-agrivibe-green to-emerald-500 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-xl transition-all"
                    >
                      <Download className="w-5 h-5" />
                      Download Now
                    </button>
                  ...
                  !showSuccess && (
                    <>
                      ...
                      <div className="mt-4 p-4 bg-gray-50 rounded-xl text-center">
                        ...
                        <p className="text-3xl font-bold text-agrivibe-green">
                          KES {guide.price}
                        </p>
                        ...
                        <div className="mt-4 space-y-4">
                          ...
                          {paymentMethod === "mpesa" && (
                            <div className="overflow-hidden">
                              ...
                              <p className="text-xs text-gray-400 mt-1">
                                Enter phone number without the + sign
                              </p>
                            </div>
                          )}
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-yellow-600 mt-0.5">
                                    Payment will be deducted from your AgriVibe
                                    Wallet balance.
                                  </p>
                                </div>
                              </div>
