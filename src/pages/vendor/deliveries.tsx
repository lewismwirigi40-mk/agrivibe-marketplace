// src/pages/vendor/deliveries.tsx
...
export default function VendorDeliveries() {
  ...
  const fetchDeliveries = async () => {
    try {
      ...
      // ✅ CORRECT: Uses the vendor deliveries endpoint
      const response = await api.get("/deliveries/vendor");
      ...
      setStats({
        ...
        assigned: deliveriesData.filter((d: any) => d.status === "assigned")
          .length,
        inTransit: deliveriesData.filter(
          (d: any) => d.status === "picked_up" || d.status === "in_transit",
        ).length,
        delivered: deliveriesData.filter((d: any) => d.status === "delivered")
          .length,
        failed: deliveriesData.filter(
          (d: any) => d.status === "failed" || d.status === "cancelled",
        ).length,
      });
    } catch (error: any) {
  ...
  // Format driver name from backend data
  const getDriverName = (delivery: any) => {
    if (delivery.driver) {
      return (
        `${delivery.driver.first_name || ""} ${delivery.driver.last_name || ""}`.trim() ||
        "Driver"
      );
    }
  ...
  const getCustomerName = (delivery: any) => {
    if (delivery.order?.customer) {
      return (
        `${delivery.order.customer.first_name || ""} ${delivery.order.customer.last_name || ""}`.trim() ||
        "Customer"
      );
    }
    return "Customer";
        ...
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-4">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search by order number, address, or driver..."
                value={searchTerm}
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <Link
                            href={`/vendor/orders/${delivery.order_id}`}
                            className="font-bold text-gray-900 hover:text-agrivibe-green transition-colors"
                          >
                            #
                            {delivery.order?.order_number ||
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              <span className="font-semibold text-agrivibe-green">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {/* ✅ FIXED: Go to order detail page instead of delivery detail */}
                        <Link
                          href={`/vendor/orders/${delivery.order_id}`}
