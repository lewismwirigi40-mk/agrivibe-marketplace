...
export default function VendorLayout({ children }: VendorLayoutProps) {
  ...
  const fetchVendorData = async () => {
    try {
      ...
      setStoreName(vendor?.business_name || "My Store");
      ...
      setStats({
        todaySales: statsData.todaySales || statsData.totalRevenue || 0,
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await api.get("/vendor/notifications?limit=5");
                ...
                <AnimatePresence>
                  {activeDropdown === "notifications" && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-white/10 overflow-hidden z-50"
                    >
                      ...
                      <div className="max-h-80 overflow-y-auto">
                        {notifications.length === 0 ? (
                          ...
                          notifications.map((n) => (
                            <div
                              key={n.id}
                              className={`p-4 border-b border-gray-50 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors ${!n.read ? "bg-yellow-500/5" : ""}`}
                            >
              ...
              <AnimatePresence>
                {activeDropdown === "profile" && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    className="absolute right-4 top-16 mt-2 w-56 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-white/10 overflow-hidden z-50"
                  >
                    <div className="p-4 border-b border-gray-100 dark:border-white/10">
                    ...
                    <div className="p-2">
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors text-red-500"
                      >
          ...
          <nav className="flex-1 p-4 space-y-1">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`w-5 h-5 ${isActive ? "text-yellow-500" : "group-hover:text-gray-900 dark:group-hover:text-white"}`}
                  />
          ...
          <div className="p-4 border-t border-gray-100 dark:border-white/10">
            <div className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 rounded-2xl p-4 border border-yellow-500/20">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                {vendorData?.business_name || storeName}
              </p>
